...
```{r data_directory, results = 'hide', eval = TRUE}
...
```{r scrape_prep, results = 'hide', eval = FALSE}
scrape_sql <- 'SELECT
                   ds.Country
                   ,ds.State
                   ,ds.City
...
```{r scrape_plot, fig.width = 12, fig.height = 8}
...
```{r listings_growth, fig.width = 12, fig.height = 14}
load('data/scrape.RData')
...
Of note is the difference in listings growth in Europe (Paris and London) against the United States (New York City and Los Angeles). Between February 02, 2016 and July 03, 2016, listings in Paris grew by `r formatC(parend - parstart, digits = 0, format = 'd', big.mark = ',')`, equaling a linear growth rate of `r formatC(((parend - parstart) / as.numeric((as.Date('2016-07-03') - as.Date('2016-02-02'))) * 30), digits = 1, format = 'f', big.mark = ',')` listings per month. Between June 02, 2016 and March 04, 2017, listings in London grew by `r formatC(lonend - lonstart, digits = 0, format = 'd', big.mark = ',')`, equaling a linear growth rate of `r formatC(((lonend - lonstart) / as.numeric((as.Date('2017-03-04') - as.Date('2016-06-02'))) * 30), digits = 1, format = 'f', big.mark = ',')` listings per month.

In the United States, growth seems to have slowed. Between July 02, 2016 and March 02, 2017, listings in New York City grew by `r formatC(nyend - nystart, digits = 0, format = 'd', big.mark = ',')`, equaling a linear growth rate of `r formatC(((nyend - nystart) / as.numeric((as.Date('2017-03-02') - as.Date('2016-06-02'))) * 30), digits = 1, format = 'f', big.mark = ',')` listings per month. Between August 03, 2016 and March 02, 2017, listings in Los Angeles shrunk by `r formatC(laend - lastart, digits = 0, format = 'd', big.mark = ',')`, equaling a linear growth rate of `r formatC(((laend - lastart) / as.numeric((as.Date('2017-03-02') - as.Date('2016-08-03'))) * 30), digits = 1, format = 'f', big.mark = ',')` listings per month.

Listings per capita for the United States cities is `r formatC(nyend / 19567471, digits = 5, format = 'f')` for a population of 19.6m in New York City and `r formatC(laend / 12828837, digits = 5, format = 'f')` for a population of 12.8m in Los Angeles as of the last scrape in the above calculations (populations equal to metro area populations from 2010 US Census). Listings per capita for the European cities is `r formatC(parend / 12014814, digits = 5, format = 'f')` for a population of 12.0m in Paris and `r formatC(lonend / 13659641, digits = 5, format = 'f')` for a population of 13.7m in London as of the last scrape in the above calculations (populations equal to metro area populations from Eurostat 2014).
...
```{r host_prep, results = 'hide', eval = FALSE}
host_sql <- 'SELECT
                 a.Country
                 ,a.State
                 ,a.City
...
```{r host_growth, fig.width = 12, fig.height = 14}
...
```{r power_host, fig.width = 12, fig.height = 14}
load('data/host.RData')
...
```{r mysql_disconnect, results = 'hide', eval = TRUE}
